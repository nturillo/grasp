...
use syn::{Error, FnArg, ItemFn, LitStr, Meta, Token, parse_macro_input, punctuated::Punctuated};
...
pub fn register(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    match return_type.as_str() {
        ...
        _ => { return new_error(format!("Expected one of [Value, Vertex, VertexList, Edge, EdgeList, None], found {}", return_type)); }
    }
    ...
    for (i, param) in &mut input.sig.inputs.iter_mut().skip(1).enumerate() {
        if let FnArg::Typed(p_data) = param {
            ...
            for attr in p_data.attrs.drain(..) {
                if !attr.path().is_ident("param") {
                    moved.push(attr);
                } else {
                    let mut values = match attr.parse_args_with(Punctuated::<LitStr, Token![,]>::parse_terminated).map_err(|e| new_error(e.to_string())) {
                        Err(e) => {return e;}
                        Ok(val) => val.into_iter(),
                    };

                    if values.len() != 2 {
                        return new_error(format!("Expected 2 arguments, found {}", values.len()));
                    }

                    let disp_name = values.next().unwrap().value();
                    let basic_type_str = values.next().unwrap().value();
