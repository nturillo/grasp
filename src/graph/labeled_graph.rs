use super::{DiGraph, EdgeID, GraphTrait, SimpleGraph, VertexID, VertexMap, graph_ops::*};
...
pub trait LabeledGraph: AsRef<Self::GraphType> + AsMut<Self::GraphType> {
    ...
    fn set_vertex_label(
        &mut self,
        v: VertexID,
        label: Self::VertexData,
    ) -> Option<Self::VertexData>;
...
impl<G: LabeledGraph> GraphTrait for G {
    ...
    fn add_neighbors(&mut self, v: VertexID, nbhrs: impl Iterator<Item = VertexID>) {
        self.as_mut().add_neighbors(v, nbhrs)
    }
...
impl<G: LabeledGraph> GraphOps for G
where
    G::GraphType: GraphOps,
{
    fn subgraph_vertex(
        &self,
        vertices: impl IntoIterator<Item = VertexID>,
        graph_builder: impl FnOnce() -> Self,
    ) -> Self {
        let subgraph = self
            .as_ref()
            .subgraph_vertex(vertices, || graph_builder().to_graph());
    ...
    fn subgraph_edges(
        ...
        let subgraph = self
            .as_ref()
            .subgraph_edges(edges, || graph_builder().to_graph());
        let mut subgraph = Self::from_graph(subgraph);
        subgraph.fill_vertex_labels(|vertex| self.get_vertex_label(vertex).cloned());
        subgraph.fill_edge_labels(|edge| self.get_edge_label(edge).cloned());
        subgraph
    ...
    fn merge(
        ...
        let (merged, self_map, other_map) = self
            .as_ref()
            .merge(other.as_ref(), || graph_builder().to_graph());
        let mut merged = Self::from_graph(merged);

        merged.set_vertex_labels(
            self.vertex_labels()
                .filter_map(|(vertex, label)| Some((*self_map.get(vertex)?, label.clone()))),
        );
        merged.set_vertex_labels(
            other
                .vertex_labels()
                .filter_map(|(vertex, label)| Some((*other_map.get(vertex)?, label.clone()))),
        );
    ...
    fn complement(&self, graph_builder: impl FnOnce() -> Self) -> Self {
        let mut complement =
            Self::from_graph(self.as_ref().complement(|| graph_builder().to_graph()));
        complement.set_vertex_labels(self.vertex_labels().map(|(v, l)| (*v, l.clone())));
        complement
    }
}
impl<G: LabeledGraph> SimpleGraphOps for G
where
    G::GraphType: SimpleGraphOps,
{
    fn join(
        &self,
        other: &Self,
        ...
        let (join, self_map, other_map) = self
            .as_ref()
            .join(other.as_ref(), || graph_builder().to_graph());
        let mut join = Self::from_graph(join);
        join.set_vertex_labels(
            self.vertex_labels()
                .filter_map(|(vertex, label)| Some((*self_map.get(vertex)?, label.clone()))),
        );
        join.set_vertex_labels(
    ...
    fn product(
        ...
        other: &Self,
        graph_builder: impl FnOnce() -> Self,
    ) -> (Self, HashMap<(VertexID, VertexID), VertexID>) {
        let (product, map) = self
            .as_ref()
            .product(other.as_ref(), || graph_builder().to_graph());
...
impl<G: LabeledGraph> DiGraph for G
where
    G::GraphType: DiGraph,
{
...
#[derive(Debug)]
pub struct HashMapLabeledGraph<G, V = (), E = ()>
where
    G: SimpleGraph,
{
    pub graph: G,
...
impl<G: SimpleGraph, V: Clone, E: Clone> LabeledGraph for HashMapLabeledGraph<G, V, E> {
    ...
    fn get_edge_label(&self, edge: EdgeID) -> Option<&Self::EdgeData> {
        if self.graph.has_edge(edge) {
            Some(
                self.edge_labels
                    .get(&edge)
                    .or(self.edge_labels.get(&(edge.1, edge.0)))?,
            )
        } else {
            None
        }
    ...
    fn set_vertex_labels(
        &mut self,
        labels: impl IntoIterator<Item = (VertexID, Self::VertexData)>,
    ) {
        for (v, l) in labels {
            if !self.contains(v) {
                self.add_vertex(v);
    ...
    fn fill_edge_labels(&mut self, mut labeler: impl FnMut(EdgeID) -> Option<Self::EdgeData>) {
        for edge in self.graph.edges() {
            let Some(label) = labeler(edge) else {
                continue;
            };
            self.edge_labels.insert(edge, label);
        }
